...
serve(async (req) => {
  ...
  try {
    const { messages, totalMessages, totalUsers, topWords, topEmojis } =
      await req.json();
    ...
    const messageTexts = sampledMessages
...
`;
    ...
    console.log("Calling Gemini AI for sentiment and insights analysis...");
...
Return ONLY this exact JSON structure:
{
  ...
  "insights": [
    ...
    if (Math.abs(total - 100) > 1) {
      ...
      analysis.sentiment.positive =
        Math.round(analysis.sentiment.positive * factor * 100) / 100;
      analysis.sentiment.neutral =
        Math.round(analysis.sentiment.neutral * factor * 100) / 100;
      analysis.sentiment.negative =
        Math.round(
